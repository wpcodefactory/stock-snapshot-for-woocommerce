...
class Alg_WC_Stock_Snapshot_Settings_General extends Alg_WC_Stock_Snapshot_Settings_Section {
	...
	function get_action_scheduler_info() {
		...
		if ( 'no' === get_option( 'alg_wc_stock_snapshot_plugin_enabled', 'yes' ) || 'no' === get_option( 'alg_wc_stock_snapshot_action_scheduler', 'yes' ) ) {
			return '';
		}
		...
		if ( defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON ) {
			...
			$info .= '<strong>' .
				sprintf( __( 'Crons are disabled on your site! You need to enable them by setting %s constant to %s.', 'stock-snapshot-for-woocommerce' ),
					'<code>' . 'DISABLE_WP_CRON' . '</code>', '<code>' . 'false' . '</code>' ) .
			'</strong>';

		} elseif ( ( $next_scheduled = as_next_scheduled_action( $action ) ) && 'yes' === get_option( 'alg_wc_stock_snapshot_plugin_enabled', 'yes' ) ) {

			$info .= sprintf( __( 'Next stock snapshot is scheduled at %s (current server time is %s).', 'stock-snapshot-for-woocommerce' ),
					'<code>' . date( 'Y-m-d H:i:s', $next_scheduled ) . '</code>', '<code>' . date( 'Y-m-d H:i:s' ) . '</code>' ) .
				'<br>* ' . sprintf( __( 'Plugin uses %s to take the stock snapshots periodically.', 'stock-snapshot-for-woocommerce' ),
					'<a href="https://actionscheduler.org/" target="_blank">' . __( 'Action Scheduler', 'stock-snapshot-for-woocommerce' ) . '</a>' ) . ' ' .
				sprintf( __( 'Action Scheduler has a built in <a href="%s" target="_blank">administration screen</a> for monitoring, debugging and manually triggering scheduled actions. Search for the %s hook there.', 'stock-snapshot-for-woocommerce' ),
					admin_url( 'admin.php?page=wc-status&tab=action-scheduler' ), '<code>' . $action . '</code>' );
	...
	function get_settings() {
		...
		$plugin_settings = array(
			...
			array(
				...
				'desc'     => __( 'Enable', 'stock-snapshot-for-woocommerce' ),
				'desc_tip' => sprintf( __( 'If enabled, you can take the snapshot with %s', 'stock-snapshot-for-woocommerce' ),
